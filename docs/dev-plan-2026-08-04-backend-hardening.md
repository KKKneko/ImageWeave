...
- [ ] F1 审计表中标为「可转」的方法 100% 已转，无遗留。
- [ ] F2 不可转的方法均有中文原因注释。
- [ ] F3 `test_read_methods_do_not_take_write_lock` 存在且通过。
- [ ] F4 审计文档新增「仍在事件循环上的同步读」小节。
- [ ] F5 本任务零行为变更（diff 中不出现新的条件分支、新字段、新默认值）。
- [ ] F6 288 个基线测试全绿，加上 T5 新增的测试也全绿。
