...
- [x] M1 任务栏每个已开窗口一个按钮，文本用中文 `label`。
- [x] M2 聚焦/最小化状态在按钮上可见且有正确的 `aria-pressed`。
- [x] M3 溢出折叠在 > 6 窗口时生效，`⋯` 按钮有 `aria-expanded`/`aria-controls`。
- [x] M4 `max-width: 767px` 下全部窗口强制最大化、仅聚焦窗口可见、
      手柄 `hidden` 不可聚焦。
- [x] M5 断点往返不丢 rect、不销毁非聚焦窗口 DOM。
- [x] M6 桌面图标区不再出现三个「开发中」占位，它们移到 START 菜单分组。
- [x] M7 占位应用仍在 `app-registry.js` 注册，路由回退行为未变。
- [x] M8 上述测试要求的行为契约已被覆盖（数量不作硬性要求），新增前端测试全绿，
      288 个后端基线测试全绿。
